...
def render():
    ...
    with c1:
        if st.button("➕ New Customer", use_container_width=True):
            st.session_state.sidebar_page = "New Customer"
    with c2:
        if st.button("📖 View n8n Templates", use_container_width=True):
            st.markdown("[Open n8n Templates](https://n8n.io/workflows/)", unsafe_allow_html=False)
    with c3:
        if st.button("🐇 Code Rabbit Docs", use_container_width=True):
            st.markdown("[Code Rabbit CLI](https://coderabbit.ai)", unsafe_allow_html=False)
